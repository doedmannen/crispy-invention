...
function create(shopItem: ShopItem): Promise<ShopItem> {
    return new Promise<ShopItem>((resolve, reject) => {
        if(shopItem.isValidForCreate()){
            ...
            let params: Array<any> = [shopItem.shoppingListId, shopItem.name, shopItem.category, shopItem.quantity, shopItem.shoppingListId];
...
function readAllFromShoppingList(shoppingListId: number): Promise<Array<ShopItem>> {
    return new Promise<Array<ShopItem>>((resolve, reject) => {
        if(Number.isInteger(shoppingListId) && shoppingListId > 0){
            ...
            let sql: string = `SELECT * FROM shopitems WHERE shoppinglist_id = ?`;
...
function update(shopItem: ShopItem): Promise<ShopItem> {
    return new Promise<ShopItem>((resolve, reject) => {
        if(shopItem.isValidForUpdate()){
            ...
            let params: Array<any> = [shopItem.shoppingListId, shopItem.name, shopItem.category, shopItem.quantity, shopItem.id];
